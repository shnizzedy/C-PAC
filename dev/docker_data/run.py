...
def main():
    ...
    parser.add_argument('bids_dir',
                        ...
                        ' format s3://bucket/path/to/bidsdir to read data'
    ...
    parser.add_argument('output_dir',
                        ...
                        ' folder should be prepopulated with the results of'
                        ' the participant level analysis. Use the format '
                        ' s3://bucket/path/to/bidsdir to write data directly'
                        ' to an S3 bucket. This may require AWS S3 credentials'
    ...
    parser.add_argument('analysis_level',
                        ...
                        ' output_dir. GUI will open the CPAC gui (currently'
                        ' only works with singularity) and test_config will'
                        ' run through the entire configuration process but'
    ...
    parser.add_argument('--pipeline_file',
                        help='Path for the pipeline configuration file to use.'
    ...
    parser.add_argument('--group_file',
                        ...
                        ' s3://bucket/path/to/pipeline_file to read data'
                        ' directly from an S3 bucket. This may require AWS S3'
                        ' credentials specified via the --aws_input_creds'
    ...
    parser.add_argument('--data_config_file',
                        ...
                        ' that is to be processed. Can be generated from the'
                        ' CPAC gui. This file is not necessary if the data in'
                        ' bids_dir is organized according to the BIDS format.'
                        ' This enables support for legacy data organization'
                        ' and cloud based storage. A bids_dir must still be'
                        ' specified when using this option, but its value will'
                        ' be ignored. Use the format'
                        ' s3://bucket/path/to/data_config_file to read data'
                        ' directly from an S3 bucket. This may require AWS S3'
                        ' credentials specified via the'
                        ' --aws_input_creds option.',
                        default=None)
    ...
    parser.add_argument('--pipeline_override', type=parse_yaml,
                        action='append',
                        help='Override specific options from the pipeline'
                        ' configuration. E.g.: "maximumMemoryPerParticipant:'
                        ' 10"')
    parser.add_argument('--aws_input_creds',
                        help='Credentials for reading from S3. If not provided'
                        ' and s3 paths are specified in the data config we'
                        ' will try to access the bucket anonymously use the'
                        ' string "env" to indicate that input credentials'
                        ' should read from the environment. (E.g. when'
                        ' using AWS iam roles).',
                        default=None)
    parser.add_argument('--aws_output_creds',
                        help='Credentials for writing to S3. If not provided'
                        ' and s3 paths are specified in the output directory'
                        ' we will try to access the bucket anonymously use'
                        ' the string "env" to indicate that output'
                        ' credentials should read from the environment. (E.g.'
                        ' when using AWS iam roles).',
                        default=None)
    ...
    parser.add_argument('--mem_mb', type=float,
                        help='Amount of RAM available to the pipeline in'
    ...
    parser.add_argument('--participant_label',
                        ...
                        ' sub-<participant_label> from the BIDS spec (so it'
                        ' does not include "sub-"). If this parameter is not'
                        ' provided all participants should be analyzed.'
                        ' Multiple participants can be specified with a space'
                        ' separated list. To work correctly this should come'
                        ' at the end of the command line.',
                        nargs="+")
    parser.add_argument('--participant_ndx',
                        help='The index of the participant'
                        ' that should be analyzed. This corresponds to the'
                        ' index of the participant in the data config file.'
                        ' This was added to make it easier to accommodate SGE'
                        ' array jobs. Only a single participant will be'
                        ' analyzed. Can be used with participant label, in'
                        ' which case it is the index into the list that'
                        ' follows the participant_label flag. Use the value'
                        ' "-1" to indicate that the participant index should'
                        ' be read from the AWS_BATCH_JOB_ARRAY_INDEX'
    ...
    parser.add_argument('--bids_validator_config',
                        ...
                        'bids-validator: See '
                        'https://github.com/bids-standard/bids-validator for '
                        'more info.')
    ...
    parser.add_argument('--monitoring',
                        help='Enable monitoring server on port 8080. You need '
                             'to bind the port using the Docker flag "-p".',
                        action='store_true')
    ...
    if args.analysis_level == "gui":
        ...
        if not args.group_file or not os.path.exists(args.group_file):
            ...
            print(
                f"Starting group level analysis of data in {args.bids_dir} "
                f"using {args.group_file}")
        ...
        # get AWS credentials, if any are specified
        ...
        if args.n_cpus == 0:
            c['maxCoresPerParticipant'] = int(
                c.get('maxCoresPerParticipant', 3))
            args.n_cpus = 3
        else:
            c['maxCoresPerParticipant'] = args.n_cpus
        c['numParticipantsAtOnce'] = int(c.get('numParticipantsAtOnce', 1))
        # Reduce cores per participant if cores times particiapants is more
        # than available CPUS. n_cpus is a hard upper limit.
        ...
        if not args.data_config_file:
            ...
            from bids_utils import collect_bids_files_configs, \
            ...
            # load the file as a check to make sure it is available and
            # readable
            sub_list = load_yaml_config(
                args.data_config_file, args.aws_input_creds)
        ...
        with open(data_config_file, 'w') as f:
            ...
            yaml.dump(
                sub_list, f, default_flow_style=False, Dumper=noalias_dumper)
